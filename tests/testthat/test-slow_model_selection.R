test_that("no publication bias selected", {
  ...
  tau <- 0.2
  set.seed(42)
  dat <- sim_mix(K = 300, M = 1, mu = mu, tau = tau, steps = c(0.5, 0.95), weights = c(1,1,1), one_sided = T)
...
test_that("publication bias two mixture selected", {
  skip_if_not_slow()

  mus <- c(0, 1)
  taus <- c(0.1, 0.2)
  set.seed(42)
  dat <- sim_mix(K = 300, M = 2, mu = mus, tau = taus, steps = c(0.95, 0.975), weights = c(0.2,0.5,1), one_sided = T)

  fit1 <- suppressWarnings(re_mix(dat$y, dat$sds, M = 1, chains = 2, cores = 2))
  fit2 <- suppressWarnings(re_mix(dat$y, dat$sds, M = 2, chains = 2, cores = 2))
  fit3 <- suppressWarnings(re_mix(dat$y, dat$sds, M = 3, chains = 2, cores = 2))

  fit1pb <- suppressWarnings(sel_mix(dat$y, dat$sds, M = 1, steps = c(0.95, 0.975), chains = 2, cores = 2))
  fit2pb <- sel_mix(dat$y, dat$sds, M = 2, steps = c(0.95, 0.975), chains = 2, cores = 2)  # warnings not suppressed
  fit3pb <- suppressWarnings(sel_mix(dat$y, dat$sds, M = 3, steps = c(0.95, 0.975), chains = 2, cores = 2))
